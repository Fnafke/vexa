...
const SignupForm = () => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response: Response = await AuthService.signup(
                {username, email, password} as SignupRequest
            );

            if (!response.ok) {
                setAuthenticationError("Failed to create account." + response.statusText);
                return;
            }
    ...
    return (
        ...
        <div className="w-full max-w-md rounded-[28px] border border-border bg-card p-6 shadow-[0_12px_40px_-16px_rgba(15,23,42,0.18)] sm:p-8">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Sign up to get started with Vexa</p>
            </div>
